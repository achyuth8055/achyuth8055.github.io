  ...
  async fn(inputs) {
    ...
    if (Action.INTERNAL_NOTIFIABLE_TYPES.includes(action.type)) {
      if (Action.PERSONAL_NOTIFIABLE_TYPES.includes(action.type)) {
        ...
        // For MOVE_CARD actions, also notify all board members
        if (action.type === Action.Types.MOVE_CARD) {
          const boardMemberUserIds = await sails.helpers.boards.getMemberUserIds(inputs.board.id);
          // Exclude the user who moved the card
          const otherBoardMemberUserIds = boardMemberUserIds.filter(
            (userId) => userId !== action.userId,
          );
          notifiableUserIds = _.union(notifiableUserIds, otherBoardMemberUserIds);
        }
